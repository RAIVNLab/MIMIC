...
def main(folder):
    ...
    ##########
        # gray = cv2.cvtColor(A[0], cv2.COLOR_RGB2GRAY)
        # variance = cv2.Laplacian(gray, cv2.CV_64F).var()
        # print(variance)
        # save_images(A[0], str(sample))
    ...
    #######
        ...
        if best_percent < 100:
            ...
            if BorC == "B":
                ...
                # show_corres(best_dict, A[ids[0]], B[ids[1]], str(sample))
            else:
                cv2.imwrite(f"{newpath}/0001.jpg", cv2.cvtColor(C[ids[1]], cv2.COLOR_RGB2BGR ))
                # show_corres(best_dict, A[ids[0]], C[ids[1]], str(sample))
