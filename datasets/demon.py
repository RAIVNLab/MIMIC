...
def main(folder):
  ...
  if best_percent < 100:
    ...
    cv2.imwrite(f"{store_path}/{folder}/0000.jpg", cv2.cvtColor(pair[0], cv2.COLOR_RGB2BGR))  
    cv2.imwrite(f"{store_path}/{folder}/0001.jpg", cv2.cvtColor(pair[1], cv2.COLOR_RGB2BGR))   
    save_corres(best_dict, f"{store_path}/{folder}")
